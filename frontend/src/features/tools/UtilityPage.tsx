...
const TIMEZONE_KEY = "avendia.timezone";
...
export function UtilityPage() {
  ...
  const saveTimezone = (event: ChangeEvent<HTMLSelectElement>) => {
    localStorage.setItem(TIMEZONE_KEY, event.target.value);
    setMessage("Preferencia guardada en este dispositivo");
  };
  return <main className="content-page">
    ...
    {formats ? <TemplateLibrary /> : (
      <section className="utility-panel">
        <label>
          Zona horaria de este dispositivo
          <select defaultValue={localStorage.getItem(TIMEZONE_KEY) ?? "America/Lima"} onChange={saveTimezone}>
