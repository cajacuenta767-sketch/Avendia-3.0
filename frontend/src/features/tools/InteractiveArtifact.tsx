...
function CrosswordBoard({ activity }: { activity: WorkflowActivity }) {
  ...
  return <div className="crossword-layout"><div className="puzzle-scroll"><div className="crossword-grid" style={{ gridTemplateColumns: `repeat(${crossword.cols}, 2.35rem)` }}>{Array.from({ length: crossword.rows * crossword.cols }, (_, index) => {
...
function WordSearchBoard({ activity }: { activity: WorkflowActivity }) {
  ...
  return <div className="wordsearch-layout"><div className="puzzle-scroll"><div className="wordsearch-grid" style={gridStyle}>{puzzle.letters.flatMap((row, rowIndex) => row.map((letter, colIndex) => { const key = `${rowIndex},${colIndex}`; const active = selected.includes(key); return <button type="button" aria-label={`Letra ${rowIndex + 1}-${colIndex + 1}`} className={active ? "is-selected" : ""} onClick={() => { setChecked(false); setSelected((current) => active ? current.filter((item) => item !== key) : [...current, key]); }} key={key}>{letter}</button>; }))}</div></div><div className="wordsearch-words"><h4>Palabras</h4>{activity.items.map((item) => <span className={found.some((foundItem) => foundItem.id === item.id) ? "is-found" : ""} key={item.id}>{item.answer}</span>)}</div>{puzzle.unplaced.length > 0 && <p role="alert">No se pudieron ubicar todas las palabras. Regenera el recurso antes de entregarlo.</p>}<div className="interactive-score"><button type="button" onClick={() => setChecked(true)} disabled={puzzle.unplaced.length > 0}><Check /> Comprobar selección</button>{checked && <strong>{found.length} de {activity.items.length} encontradas</strong>}</div></div>;
}
