...
function UsersView({ data, search, onSearch, role, onRole, status, onStatus, onOpen, opening }: { data: AdminUsersResponse; search: string; onSearch: (v: string) => void; role: string; onRole: (v: string) => void; status: string; onStatus: (v: string) => void; onOpen: (id: string) => void; opening: boolean }) {
  return <div className="admin-view"><section className="admin-panel admin-users-panel">
    <header className="admin-table-header"><div><h2>Gestión de usuarios</h2><p>{formatNumber.format(data.total)} cuentas coinciden con los filtros. {formatNumber.format(data.teachers_count)} docentes y {formatNumber.format(data.admins_count)} administradores registrados.</p></div><CsvButton users={data.items}/></header>
    <div className="admin-filters"><label className="admin-search"><Search/><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Buscar nombre, correo o institución"/></label><select value={role} onChange={(event) => onRole(event.target.value)} aria-label="Filtrar por rol"><option value="">Todos los roles</option><option value="teacher">Docentes</option><option value="admin">Administradores</option></select><select value={status} onChange={(event) => onStatus(event.target.value)} aria-label="Filtrar por estado"><option value="">Todos los estados</option><option value="true">Activas</option><option value="false">Inactivas</option></select></div>
    <div className="admin-table-scroll"><table className="admin-table"><thead><tr><th>Cuenta</th><th>Perfil educativo</th><th>Actividad</th><th>IA</th><th>Estado</th><th/></tr></thead><tbody>{data.items.map((user) => <tr key={user.id}><td><span className="admin-user-cell"><i>{user.full_name.split(" ").slice(0, 2).map((name) => name[0]).join("")}</i><span><strong>{user.full_name}</strong><small>{user.email}</small><small>{user.school_name}</small></span></span></td><td><strong>{user.education_modality} · {user.education_level}</strong><small>{user.grade}</small></td><td><strong>{user.documents_count} documentos</strong><small>{user.events_count} eventos</small></td><td><strong>{formatNumber.format(user.ai_credits_balance)} créditos</strong><small>{user.ai_generations} generaciones</small></td><td><span className={`admin-pill ${user.is_active ? "admin-pill--success" : "admin-pill--muted"}`}>{user.is_active ? "Activa" : "Inactiva"}</span><small>{user.role === "admin" ? "Administrador" : "Docente"}</small></td><td><button className="admin-row-action" onClick={() => onOpen(user.id)} disabled={opening}>Gestionar <ChevronRight/></button></td></tr>)}</tbody></table></div>
    <div className="admin-user-cards" aria-label="Usuarios">
      {data.items.map((user) => <article className="admin-user-card" key={user.id}>
        <header><span className="admin-user-cell"><i>{user.full_name.split(" ").slice(0, 2).map((name) => name[0]).join("")}</i><span><strong>{user.full_name}</strong><small>{user.email}</small></span></span><span className={`admin-pill ${user.is_active ? "admin-pill--success" : "admin-pill--muted"}`}>{user.is_active ? "Activa" : "Inactiva"}</span></header>
        <p>{user.school_name}</p>
        <dl><div><dt>Perfil</dt><dd>{user.education_modality} · {user.education_level} · {user.grade}</dd></div><div><dt>Actividad</dt><dd>{user.documents_count} documentos · {user.events_count} eventos</dd></div><div><dt>Inteligencia artificial</dt><dd>{formatNumber.format(user.ai_credits_balance)} créditos · {user.ai_generations} generaciones</dd></div><div><dt>Rol</dt><dd>{user.role === "admin" ? "Administrador" : "Docente"}</dd></div></dl>
        <button className="admin-row-action admin-user-card__action" onClick={() => onOpen(user.id)} disabled={opening}>Gestionar cuenta <ChevronRight/></button>
      </article>)}
    </div>
    {!data.items.length ? <div className="admin-empty">No hay cuentas que coincidan con estos filtros.</div> : null}
  </section></div>;
}
